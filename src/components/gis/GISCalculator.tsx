  ...
  return (
    <div className="card p-5">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="font-semibold">GIS 净收入测试</h3>
        {result.eligible ? <Badge color="green">✅ 有资格</Badge> : <Badge color="red">❌ 超出门槛</Badge>}
      </div>
      ...
      <div className="mt-3 rounded-lg bg-slate-50 p-3">
        <label className="label">模拟：本年取用资本利得</label>
        <input
          type="range"
        ...
        />
        <div className="text-xs text-slate-500">取用 {formatCurrency(capitalGains)} → 净收入 +{formatCurrency(capitalGains * 0.5)}（50%计入）</div>
      </div>

      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-lg bg-slate-50 p-3">
          <div className="text-xs text-slate-500">净收入合计</div>
        ...
        <div className={`rounded-lg p-3 ${result.eligible ? 'bg-emerald-50' : 'bg-rose-50'}`}>
          <div className="text-xs text-slate-500">GIS 月额</div>
      ...
      {room > 0 ? (
        <p className="mt-3 text-sm text-slate-500">
          💡 您当前净收入 {formatCurrency(result.netIncome)}，距离 GIS 归零门槛还有{' '}
          <span className="font-medium text-slate-800">{formatCurrency(room)}</span> 空间。
        </p>
      ) : (
