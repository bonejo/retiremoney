...
export default function Estate() {
  ...
  return (
    <Page title="遗产规划">
      ...
      {rows.length === 0 ? (
        <div className="card p-10 text-center text-slate-400">添加房产或投资后显示遗产分析。</div>
      ) : (
        <div className="card overflow-x-auto p-5">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left text-slate-500">
                <th className="py-2 pr-4">资产类型</th>
                <th className="py-2 pr-4">预计市值</th>
                <th className="py-2 pr-4">资本利得税</th>
                <th className="py-2 pr-4">Probate费</th>
                <th className="py-2 pr-4">净传承金额</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
              ...
              <tr className="border-t-2 border-slate-200 font-semibold">
                <td className="py-2 pr-4">{totals.label}</td>
