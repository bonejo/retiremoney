    ...
    <Page title="资产预测">
      {depletion && (
        <div className="mb-4 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          <span>⚠️</span>
          <span>
            <span className="font-semibold">{depletion.age}岁</span>
            时可提取投资耗尽，当年资金缺口约{' '}
            <span className="font-semibold">{formatCurrency(depletion.unfundedShortfall)}</span>。
            此后图表假设缺口未被填补（净资产只剩房产等不可提取资产）——实际需要提前规划：
            出售房产、反向按揭 / HELOC，或削减支出。
          </span>
        </div>
      )}
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="card p-5 lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="font-semibold">
              {last ? `${last.age}岁 总净资产 ${formatCurrency(last.netWorth)}` : '资产曲线'}
            </h3>
        ...
        <div className="card space-y-5 p-5">
          <h3 className="font-semibold">参数调节</h3>
          <SliderInput label="房产年增值率" value={propApp} onChange={setPropApp} max={0.08} />
          <SliderInput label="非注册 ETF 年回报" value={nonReg} onChange={setNonReg} max={0.12} />
          <SliderInput label="TFSA 年回报" value={tfsa} onChange={setTfsa} max={0.12} />
        </div>
      </div>

      <div className="card mt-6 p-5">
        <h3 className="mb-1 font-semibold">数据表（每5年）</h3>
        <p className="mb-3 text-xs text-slate-400">
          现金收入（含股息/利息/租金/OAS/CPP/GIS）先支付支出，缺口按提取优先级从投资账户取款。
          取款仅资本利得的50%计入应税收入（TFSA免税、RRSP全额）；所得税按次年支出计入，
          上一年提取产生的应税收入计入当年 GIS 测试。账户取空后剩余缺口列为「未补缺口」。
        </p>
          ...
          <table className="w-full whitespace-nowrap text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left text-slate-500">
                ...
                <th className="py-2 pr-4">总资产</th>
                <th className="py-2 pr-4">TFSA</th>
                <th className="py-2 pr-4">非注册</th>
                <th className="py-2 pr-4">现金收入</th>
                <th className="py-2 pr-4">支出</th>
                <th className="py-2 pr-4">取款</th>
                <th className="py-2 pr-4">资本利得</th>
                <th className="py-2 pr-4">应税收入</th>
                <th className="py-2 pr-4">所得税</th>
                <th className="py-2 pr-4">未补缺口</th>
              </tr>
            </thead>
            <tbody>
              {tableRows.map((r) => (
                <tr key={r.year} className="border-b border-slate-50">
                  <td className="py-2 pr-4 font-medium">{r.age}岁</td>
