  ...
  return (
    <Page title="主仪表板">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard label="总净资产" value={formatCurrency(today?.netWorth ?? 0)} accent="brand" />
        <KPICard
          label="月净现金流"
          ...
          sub={
            supplementActive && plan.totalWithdrawal > 0
              ? `缺口由投资提取补足 ${formatCurrency(plan.totalWithdrawal / 12)}/月`
              : `收入 ${formatCurrency(monthlyIncome)} − 支出 ${formatCurrency(monthlyExpense)}（含所得税月摊）`
          }
        />
        <KPICard
          label="年度预估税费"
          value={formatCurrency(annualTax)}
          sub={supplementActive && withdrawalTax > 0 ? '所得税（含提取被动收入）+ 地税' : '所得税 + 地税'}
        />
        <KPICard label="TFSA 余额" value={formatCurrency(tfsaTotal)} accent="positive" />
      </div>

      <div className="card mt-6 p-5">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="font-semibold">资产预测</h3>
        ...
        {properties.length === 0 && investments.length === 0 ? (
          <p className="py-10 text-center text-slate-400">添加房产或投资账户后显示预测曲线。</p>
        ) : (
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="card p-5">
          <h3 className="mb-3 font-semibold">本月收支摘要</h3>
          <Row label="租金收入（毛租金−空置）" value={cashflow.rentalIncome / 12} positive />
          <Row label="OAS" value={derived.oasMonthly} positive />
          {derived.cppMonthly > 0 && <Row label="CPP" value={derived.cppMonthly} positive />}
          <Row
            label={gis.gisMonthly < derived.gis.gisMonthly ? 'GIS（已计提取收入影响）' : 'GIS'}
            value={gis.gisMonthly}
            positive
          />
          <Row label="投资股息/利息" value={(derived.eligibleDividends + derived.interest) / 12} positive />
          {cashflow.familyLoanRepayments > 0 && (
            <Row label="家庭借款还款" value={cashflow.familyLoanRepayments / 12} positive />
          )}
          <div className="my-2 border-t border-slate-100" />
          <Row label="房产支出（含归属支出）" value={-cashflow.propertyExpenses / 12} />
          <Row label="个人支出" value={-cashflow.personalExpenses / 12} />
          <Row label="所得税（月摊）" value={-baseTax.totalTax / 12} />
          <div className="my-2 border-t border-slate-100" />
          <Row label="净现金流" value={netCashFlow} bold positive={netCashFlow >= 0} />
              ...
              {supplementActive && (
                <div className="mt-2 space-y-0.5 text-sm">
                  {plan.rows.map((r) => (
                    <Row
                      key={r.investment.id}
                      label={`提取：${r.investment.accountName || r.investment.type}`}
                      value={r.withdrawal / 12}
                      positive
                    />
                  ))}
                  {withdrawalTax > 0 && (
                    <Row label="提取被动收入增加的所得税（分档计算）" value={-withdrawalTax / 12} />
                  )}
                  <div className="my-1 border-t border-slate-200" />
                  {plan.uncovered > 0.01 ? (
                    <Row label="补足后仍缺" value={-plan.uncovered / 12} bold />
                  ) : plan.rows.length > 0 ? (
                    <Row label="补足后净现金流" value={0} bold positive />
                  ) : (
                    <p className="pt-1 text-xs text-amber-600">
                      没有可提取的账户——请在「投资账户」为账户设置「提取优先级」。
                    </p>
                  )}
                  {plan.rows.some((r) => r.realizedGain > 0 || r.ordinaryIncome > 0) && (
                    <p className="pt-1 text-xs text-slate-400">
                      提取产生的应税收入已计入所得税与 GIS 净收入测试。
                    </p>
                  )}
                </div>
              )}
        ...
        <div className="card p-5">
          <h3 className="mb-3 font-semibold">关键提醒</h3>
          <div className="space-y-2 text-sm">
            {depletion && (
              <Reminder
                color="⚠️"
                text={`预测 ${depletion.age} 岁时可提取投资耗尽，当年资金缺口约 ${formatCurrency(depletion.unfundedShortfall)}——需提前规划（卖房、反向贷款或削减支出）`}
              />
            )}
            <Reminder color="🔴" text={`年地税约 ${formatCurrency(annualPropertyTax)}（通常7月到期）`} />
            {mortgageReminder.map((m, i) => (
              <Reminder
                ...
                text={
                  m.stats.valid
                    ? `${m.name} Mortgage 预计 ${m.age.toFixed(0)} 岁还清`
                    : `${m.name} Mortgage 月供不足以覆盖利息`
                }
              />
            ))}
            <Reminder
              ...
              text={
                gis.eligible
                  ? `GIS 有资格（净收入 ${formatCurrency(gis.netIncome)} < 门槛 ${formatCurrency(gisThreshold)}，含提取收入）`
                  : `GIS 超出门槛（净收入 ${formatCurrency(gis.netIncome)}，含提取收入）`
              }
            />
            <Reminder
              color="🔵"
              text={`TFSA 可存额度 ${formatCurrency(
      ...
      {(properties.length > 0 || expenses.length > 0) && (
        <div className="card mt-6 p-5">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="font-semibold">年度收支总览</h3>
            <span
              className={`text-lg font-semibold ${netCashFlow >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
            >
              {netCashFlow < 0 ? '−' : '+'}{formatCurrency(Math.abs(netCashFlow * 12))}/年
            </span>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <div className="mb-1 text-xs font-medium uppercase tracking-wide text-slate-400">按房产</div>
              {cashflow.propertyRows.length === 0 ? (
                <p className="py-2 text-sm text-slate-400">暂无房产。</p>
              ) : (
                cashflow.propertyRows.map(({ property, balance }) => (
                  <Row
                    key={property.id}
                    label={`${property.name || '未命名'}（租金 − 全部持有成本）`}
                    value={balance.net}
                    positive={balance.net >= 0}
                  />
                ))
              )}
              <Row label="个人支出（未归属房产）" value={-cashflow.personalExpenses} />
              <Row label="所得税" value={-baseTax.totalTax} />
            </div>
            <div>
              <div className="mb-1 text-xs font-medium uppercase tracking-wide text-slate-400">其他收入</div>
              <Row label="OAS + CPP + GIS" value={(derived.oasMonthly + derived.cppMonthly + gis.gisMonthly) * 12} positive />
              <Row label="投资股息/利息" value={derived.eligibleDividends + derived.interest} positive />
              {cashflow.familyLoanRepayments > 0 && (
                <Row label="家庭借款还款（今年）" value={cashflow.familyLoanRepayments} positive />
              )}
            </div>
          </div>
        </div>
